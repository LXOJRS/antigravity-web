...
- **Version: V120.1** (style.css?v=120, script.js?v=100 across all 5 HTML files). V120.1 = rhythm tuning of the V120 Featured work collage: PG media now has `min-height: 75vh` so PG dominates the first viewport when the section scrolls in; CLM negative top-margin reduced -160 → -120px for a small corner kiss (~18% of PG height) instead of a half-section overlap; PG brand bumped to `z-index: 3` with PG article's own z-index removed, plus `clearProps: 'transform,opacity'` + `once: true` on the .featured-case GSAP reveal so the inline identity transform that creates a stacking context gets wiped after animation — PG brand text now renders above CLM video instead of being trapped inside PG article's transform-induced stacking context. Mobile (≤768px) is untouched (min-height reset to 0, positive margin restored).
...
5. **Cache bumping**: always bump `style.css?v=` and `script.js?v=` on every HTML file when those assets change. Current: `style.css?v=120`, `script.js?v=100`.
