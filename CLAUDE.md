...
- **Version: V116** (style.css?v=115, script.js?v=96 across all 5 HTML files). V116 = post-V114 round 2: asset swaps (lens-ai top banner, podcast top banner, real Weavy workflow screenshot replaces the dashed-stripe placeholder, podcast featured-episode YouTube embed replaces the static thumbnail) plus the real fix for the V115 R1 "How I work" cutoff (title was clipping because `text-transform: uppercase` made "HOW I WORK" wider than its 4-col grid cell — bumped grid placement to span 5).
...
4. **Cache bumping**: always bump `style.css?v=` and `script.js?v=` on every HTML file when those assets change. Current: `style.css?v=115`, `script.js?v=96`.
