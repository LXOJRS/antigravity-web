...
- **Version: V119** (style.css?v=118, script.js?v=98 across all 5 HTML files). V119 = About-page redesign of two sections: "Things I Build" gets icy-blue rounded-pill bullets with a horizontal swipe-in animation (pill 1 from left, pill 2 from right, pill 3 from left, GSAP + ScrollTrigger); "Lens AI" block restructured with display-scale wordmark top-right (keeps Alte Haas), an icy-blue tagline ("Lens AI is my visual production label."), and rewritten body copy with `.highlight` spans on "art directed" and "pipeline-built". Site-wide vertical spacing between part2b rows bumped 64px to push into the 128-200px breathing tier. Earlier V118 = Overused Grotesk replaces Inter; V117 = brand color shift to `#121212` / `#fafafa`; V116 = asset swaps + "How I work" cutoff fix.
...
5. **Cache bumping**: always bump `style.css?v=` and `script.js?v=` on every HTML file when those assets change. Current: `style.css?v=118`, `script.js?v=98`.
