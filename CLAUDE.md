...
- **Version: V120** (style.css?v=119, script.js?v=99 across all 5 HTML files). V120 = lens-ai.html "Featured work" collage replaces the V113-era `.in-production` grid. Two cases (Promptgorillas, Chris le More) with asymmetric brand-name vs. video placement per case, CSS-only 2D corner overlap between cases (Chris le More video crosses 165px into Promptgorillas video's right edge horizontally + 160px negative top-margin vertically, z-index lifts CLM above PG). Mobile drops the collage and stacks each case vertically with brand above video (`order: -1` on PG brand reverses HTML source order on mobile only). Chris le More video is now a Cloudinary mp4 replacing the static image. Earlier V119 = About-page Things I Build pills + Lens AI block redesign; V118 = Overused Grotesk replaces Inter; V117 = brand color shift to `#121212` / `#fafafa`.
...
5. **Cache bumping**: always bump `style.css?v=` and `script.js?v=` on every HTML file when those assets change. Current: `style.css?v=119`, `script.js?v=99`.
